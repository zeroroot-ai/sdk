...
// tsserverClient wraps the base LSP client with typescript-language-server-specific functionality.
...
// newTsserverClient creates a new typescript-language-server client for the given workspace.
func newTsserverClient(ctx context.Context, tsserverPath, workspaceRoot string, logger *slog.Logger) (*tsserverClient, error) {
	// Find typescript-language-server binary
	if tsserverPath == "" {
		path, err := exec.LookPath("typescript-language-server")
		if err != nil {
			return nil, fmt.Errorf("typescript-language-server not found in PATH: %w", err)
		}
		tsserverPath = path
	}

	// Create command with --stdio flag for JSON-RPC over stdio
	cmd := exec.CommandContext(ctx, tsserverPath, "--stdio")

	// Create base client
	baseClient, err := newLSPClient(cmd, logger.With("component", "tsserver"))
	if err != nil {
		return nil, err
	}
...
func (c *tsserverClient) initialize(ctx context.Context) error {
	// Send initialize request with typescript-language-server-specific capabilities
	...
	if err := json.Unmarshal(result, &initResult); err == nil {
		c.logger.Debug("typescript-language-server initialized", "capabilities", initResult["capabilities"])
	}
...
func (c *tsserverClient) handlePublishDiagnostics(params json.RawMessage) {
...
// openDocument notifies typescript-language-server that a document has been opened.
func (c *tsserverClient) openDocument(ctx context.Context, filePath, content string) error {
	if !c.initialized.Load() {
		return ErrLSPNotInitialized
	}

	// Determine languageId based on file extension
...
// closeDocument notifies typescript-language-server that a document has been closed.
func (c *tsserverClient) closeDocument(filePath string) error {
	if !c.initialized.Load() {
		return ErrLSPNotInitialized
	}
...
// It waits for diagnostics to be published by typescript-language-server after opening the document.
func (c *tsserverClient) getDiagnostics(ctx context.Context, filePath string, timeout time.Duration) ([]codegen.Diagnostic, error) {
	if !c.initialized.Load() {
...
// waitForReady blocks until the typescript-language-server client is initialized and ready.
func (c *tsserverClient) waitForReady(ctx context.Context) error {
	select {
	case <-c.readyCh:
		return nil
	case <-ctx.Done():
...
// shutdown performs graceful shutdown of the typescript-language-server client.
func (c *tsserverClient) shutdown(ctx context.Context) error {
	if !c.initialized.Load() {
		return c.close()
	}
